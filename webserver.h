...
<div class="container">
    ...
    <h2>Señal Temporal (1024 muestras)</h2>
    <canvas id="timeGraph"></canvas>

    <h2>Espectro de Magnitud FFT (0 — 500 Hz)</h2>
...
async function fetchBatch() {
    ...
    try {
        ...
        // Esperados: (1024 * 3) + (512 * 3) = 4608 floats
        ...
        // 2. Extraer FFT de los tres ejes
        ...
        for (let i = 0; i < FFT_BINS; i++) {
            ...
            // Ignorar bin 0 y 1 para descartar la componente DC (gravedad)
            if (i > 1) {
...
function drawFFT() {
    ...
    // Escalar dinámico global para los 3 ejes (omitiendo DC)
    let maxVal = 5;
    ...
    for (let freq = 100; freq <= 500; freq += 100) {
        ...
        fftCtx.fillText(freq + " Hz", x + 4, 14);
    }

    // Dibujar curvas de espectro superpuestas
